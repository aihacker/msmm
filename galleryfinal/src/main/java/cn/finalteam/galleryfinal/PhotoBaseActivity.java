...
public abstract class PhotoBaseActivity extends Activity implements EasyPermissions.PermissionCallbacks {
     ...
     * 拍照
     */
    protected void takePhotoAction(Context context) {

        //判断是否有相机
        boolean b = checkCameraDevice(context);
        if(!b){
            Toast.makeText(context,"该设备不支持相机",Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (requestCode == GalleryFinal.TAKE_REQUEST_CODE) {
            if (resultCode == RESULT_OK && mTakePhotoUri != null) {
                ...
                if (new File(mTakePhotoPath).exists()) {
                    ...
                    info.setPhotoPath(mTakePhotoPath);
                    updateGallery(mTakePhotoPath);
                    takeResult(info);
                } else {
                    takePhotoFailure();
                }
