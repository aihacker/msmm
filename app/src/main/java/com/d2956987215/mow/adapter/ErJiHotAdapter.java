...
public class ErJiHotAdapter extends BaseQuickAdapter<ErJiListResponse.DataBeanX.DataBean, BaseViewHolder> {
    public ErJiHotAdapter(@LayoutRes int layoutResId, @Nullable List<ErJiListResponse.DataBeanX.DataBean> data) {
        super(layoutResId, data);
    }

    @Override
    protected void convert(BaseViewHolder holder, final ErJiListResponse.DataBeanX.DataBean item) {
        ...
        } else if ("J".endsWith(item.getShoptype())) {
            iv_logo.setBackground(mContext.getResources().getDrawable(R.mipmap.jd_icon_small));
        } else if ("P".endsWith(item.getShoptype())) {
            iv_logo.setBackground(mContext.getResources().getDrawable(R.mipmap.pdd_icon_small));
        } else {
...
//                        mContext.startActivity(intent);
                    ...
                    if (tv_fenxiang_title.getText().toString().contains("领券")) {
                        couponRequest(User.uid(), item.getItemid(), item.getQuan_id(), item.getPid());
                    }
                }
            });
...
//                        mContext.startActivity(intent);
                    ...
                    shareRequest(User.uid(), item.getItemid(), item.getQuan_id());
                }
            });
            holder.getView(R.id.la_shengji).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    ...
                    Intent intent = new Intent(mContext, MyHuiYuanDetailActivity.class);
                    intent.putExtra("roletype", User.upreletype());
                    mContext.startActivity(intent);
                }
            });
...
//                    Intent intent = new Intent(mContext, MyHuiYuanDetailActivity.class);
//                    intent.putExtra(MyHuiYuanDetailActivity.APPLY, User.roleType());
//                    mContext.startActivity(intent);
                }
            });
        }
                        ...
                        public void NO() {
                            ...
                            switch (response.getData().getMenu().getRighttype()) {
                                ...
                                case 2:
                                    ...
                                    break;
                                case 3:
                                    mContext.startActivity(new Intent(mContext, MyHuiYuanDetailActivity.class).putExtra("roletype", User.upreletype()));
                                    break;
                            }
    ...
    private void couponRequest(int userId, String itemid, String coupon_id, final String pids) {
        ...
        new Request<GetCouponResponse>().request(quan, "优惠券接口", mContext, true, new Result<GetCouponResponse>() {
            @Override
                        ...
                        public void NO() {
                            ...
                            switch (response.getData().getMenu().getRighttype()) {
                                ...
                                case 2:
                                    ...
                                    break;
                                case 3:
                                    mContext.startActivity(new Intent(mContext, MyHuiYuanDetailActivity.class).putExtra("roletype", User.roleType()));
                                    break;
                            }
